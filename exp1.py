...
model_savefile = "./experimento1.pth"
save_model = True
load_model = True
skip_learning = True
...
def test(game, agent):
    ...
    for test_episode in trange(test_episodes_per_epoch, leave=False):
        ...
        while not game.is_episode_finished():
            ...
            depth_buf = preprocess(game.get_state().depth_buffer)
            labels_buf = preprocess(game.get_state().labels_buffer)

            state = np.concatenate(
                (screen_buf, depth_buf, labels_buf), axis=0)
...
class DuelQNet(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)  # [32, 3, 30, 45]
        x = self.conv2(x)
        x = self.conv3(x)
        x = self.conv4(x)
        # x = x.view(-1, 192)
        x = x.view(-1, 192)  # [32, 16, 2, 6]
        x1 = x[:, :96]  # input for the net to calculate the state value
        x2 = x[:, 96:]  # relative advantage of actions in the state
        state_value = self.state_fc(x1).reshape(-1, 1)
        advantage_values = self.advantage_fc(x2)
        x = state_value + (advantage_values -
                           advantage_values.mean(dim=1).reshape(-1, 1))
...
class DQNAgent:
    ...
    def get_action(self, state):
        if np.random.uniform() < self.epsilon:
            ...
            q_n = self.q_net(state)
            action = torch.argmax(q_n).item()
            return action
    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            next_state_values = next_state_values[idx]
...
if __name__ == '__main__':
    ...
    print("Score promedio: ", score_avg)

    nombreArchivo = "experimentos1Res"
    import matplotlib.pyplot as plt
    plt.plot(score_arr)
    plt.title("Experimentación")
    plt.xlabel("Pruebas")
    plt.ylabel("Score")
